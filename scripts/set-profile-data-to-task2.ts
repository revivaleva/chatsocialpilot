#!/usr/bin/env tsx
/**
 * 「X兵隊12/18作成、プロフィール未変更」グループのデータにプロフィール情報をセットしてタスク2に登録
 * 
 * 使用方法:
 *   npx tsx scripts/set-profile-data-to-task2.ts
 * 
 * 実行内容:
 * 1. グループ内のコンテナを取得
 * 2. 各コンテナに対して未使用のプロフィール情報を取得
 * 3. x_accountsテーブルにプロフィール情報をセット
 * 4. プリセット18（プロフィール変更）のタスク2に登録
 */
...
async function main(): Promise<void> {
  ...
  console.log(`処理内容: プロフィール情報をセットしてタスク2に登録`);
  console.log(`=`.repeat(80));
  console.log('');
  ...
  // 既にタスクが登録されているコンテナを確認（プリセット18、タスク2のみ）
  ...
  if (unregisteredContainers.length === 0) {
    console.log(`すべてのコンテナにタスク2が登録済みです。`);
    process.exit(0);
  }
