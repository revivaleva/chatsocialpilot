/**
 * 今回登録した100件のアカウント以外のログインタスクを削除するスクリプト
 * 
 * 処理内容:
 * 1. 今回登録した100件のアカウントのリストを取得
 * 2. プリセット17（X Authログイン）のタスクを全件取得
 * 3. 今回登録した100件以外のアカウントに対するタスクを削除
 * 
 * 使用方法:
 *   npx tsx scripts/cleanup-unnecessary-login-tasks.ts
 */
 ...
 * プリセット17の全タスクを取得
 */
function getAllLoginTasks(): Task[] {
  return query<Task>(
    'SELECT id, runId, overrides_json FROM tasks WHERE preset_id = 17',
    []
  );
}
...
function main() {
  ...
  // プリセット17の全タスクを取得
  const allTasks = getAllLoginTasks();
  ...
  for (const task of allTasks) {
    try {
      ...
      // JSON解析エラーは削除対象に含める（不正なデータの可能性）
      console.warn(`⚠ タスクの解析エラー (Run ID: ${task.runId}): ${e}`);
      tasksToDelete.push(task);
    }
  }

  console.log(`保持するタスク: ${tasksToKeep.length}件`);
  console.log(`削除対象のタスク: ${tasksToDelete.length}件\n`);

  if (tasksToDelete.length === 0) {
    console.log('✓ 削除対象のタスクはありません');
    return;
  }
  ...
  for (const task of tasksToDelete) {
    try {
      const overrides = JSON.parse(task.overrides_json || '{}');
      const containerName = String(overrides.container_name || '不明');
  ...
  console.log(`保持: ${tasksToKeep.length}件`);
  console.log('='.repeat(60));
