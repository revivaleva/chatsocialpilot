#!/usr/bin/env node
/**
 * CommonJS version of pattern runner for environments where package.json sets "type": "module"
 */
const fs = require('fs');
...
async function main() {
  ...
  if (!argv[0]) {
    console.error('Usage: node scripts/pattern_runner.cjs <pattern.json> [--postUrl=...] [--accountUrl=...]');
    process.exit(1);
  }
