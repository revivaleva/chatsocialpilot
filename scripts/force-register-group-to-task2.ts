#!/usr/bin/env tsx
/**
 * 「X兵隊12/18作成、プロフィール未変更」グループの全コンテナにプロフィール情報をセットしてタスク2に登録
 * （既存登録判定をスキップして強制登録）
 * 
 * 使用方法:
 *   npx tsx scripts/force-register-group-to-task2.ts
 */
...
async function main(): Promise<void> {
  ...
  console.log(`処理内容: グループのすべてのコンテナにプロフィール情報をセットしてタスク2に登録`);
  console.log(`=`.repeat(80));
  console.log('');
  ...
  for (let i = 0; i < containerNames.length; i++) {
    ...
    try {
      console.log(`[${i + 1}/${containerNames.length}] ${containerId}`);
