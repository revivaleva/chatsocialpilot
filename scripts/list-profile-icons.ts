 ...
 * 
 * 使用方法:
 *   tsx scripts/list-profile-icons.ts                    # 全件表示
 *   tsx scripts/list-profile-icons.ts --unused          # 未使用のみ
 *   tsx scripts/list-profile-icons.ts --used            # 使用済みのみ
 *   tsx scripts/list-profile-icons.ts --stats           # 統計情報のみ
 */
...
async function main(): Promise<void> {
  ...
  // 画像一覧を取得
  let sql = 'SELECT id, file_id, url, used, used_at, created_at FROM profile_icons';
  ...
  const icons = query<ProfileIcon>(sql);
