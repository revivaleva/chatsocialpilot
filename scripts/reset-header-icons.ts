 ...
 * 
 * 使用方法:
 *   tsx scripts/reset-header-icons.ts              # 全件リセット
 *   tsx scripts/reset-header-icons.ts <file_id>    # 特定の画像のみリセット
 */
...
async function main(): Promise<void> {
  ...
  if (input) {
    ...
    const icons = query<{ id: number; file_id: string; url: string }>(
      'SELECT id, file_id, url FROM header_icons WHERE file_id = ?',
      [fileId]
    );
    ...
    run('UPDATE header_icons SET used = 0, used_at = NULL WHERE file_id = ?', [fileId]);
    console.log(`画像をリセットしました: ${icons[0].url}`);
  } else {
    // 全件リセット
    const result = run('UPDATE header_icons SET used = 0, used_at = NULL WHERE used = 1');
