...
async function main() {
  ...
  const targetCount = 4;
  const scheduleHoursLater = 3; // 3時間後
  
  console.log(`グループ「${groupName}」から${targetCount}件のタスクを作成します...\n`);
  ...
  if (availableContainers.length < targetCount) {
    console.warn(`⚠ 未設定のコンテナが${availableContainers.length}件しかありません。${availableContainers.length}件のタスクを作成します。\n`);
  }
  
  const containersToProcess = availableContainers.slice(0, targetCount);
  ...
  console.log(`対象コンテナ数: ${containersToProcess.length}件\n`);
  ...
  const emailAccounts = query<{id: number, email_password: string, added_at: number}>(
    ...
    [containersToProcess.length]
  );
  
  if (!emailAccounts || emailAccounts.length < containersToProcess.length) {
    console.error(`✗ 未使用のメールアカウントが不足しています（必要: ${containersToProcess.length}件、利用可能: ${emailAccounts?.length || 0}件）`);
    process.exit(1);
  }
  ...
  // 予定時刻を計算（3時間後）
  const scheduledAt = Date.now() + (scheduleHoursLater * 60 * 60 * 1000);
  console.log(`予定時刻: ${new Date(scheduledAt).toLocaleString('ja-JP')}\n`);
  ...
  for (let i = 0; i < containersToProcess.length; i++) {
    ...
    console.log(`[${i + 1}/${containersToProcess.length}] コンテナ: ${containerName} (ID: ${containerId})`);
