...
const tasks = db.prepare(`
  ...
  WHERE DATE(datetime(t.scheduled_at/1000, 'unixepoch')) = '2025-12-15'
  ORDER BY t.scheduled_at, t.queue_name, t.preset_id
`).all() as { runId: string, preset_id: number, container_id: string, scheduled_at: number, queue_name: string, preset_name: string }[];

console.log(`=== 15日のタスク一覧 (合計 ${tasks.length}件) ===\n`);
...
const queueStats = db.prepare(`
  ...
  FROM tasks
  WHERE DATE(datetime(scheduled_at/1000, 'unixepoch')) = '2025-12-15'
  GROUP BY queue_name
  ORDER BY queue_name
