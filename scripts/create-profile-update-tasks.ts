/**
 * プロフィール変更タスクを全件作成するスクリプト
 * 
 * 処理内容:
 * 1. profile_dataテーブルから全アカウントのプロフィール情報を取得
 * 2. 既にタスクが作成されているアカウントを除外
 * 3. 残りのアカウントに対してプリセット18（プロフィール変更）のタスクを作成
 * 
 * 重要: container_idについて
 ...
 * 
 * 使用方法:
 *   npx tsx scripts/create-profile-update-tasks.ts
 */
...
function getTargetAccountIds(): Set<string> {
  const dataLines = [
    ...
    'mechabotan4448:rnQlcyjOQhl:takekonishin6918@outlook.com:88HvwFpLuU:NJIUQFA3OYH3AAK4:447384dd900c9df464735757e988f7d1bea64082:a1b13b83a6662e70c300ce4731a6fd9dc53e5a611ac5be5da3e1afb5f73b34f47ab1fd0c0c80ae19654d241dfd52fd983203afcc6783a5c2efefc9f351abc779ebc12fcf03812fdb6ae896e0bbc553b',
...
/**
 * 既にタスクが作成されているアカウントのcontainer_idを取得（タスク1のみ）
 */
function getExistingTaskContainerIds(): Set<string> {
  // タスク1（queue_name = 'default' または 'タスク1'）のタスクのみを確認
  // 注: DEFAULT_QUEUE_NAME = 'default' だが、UIでは 'タスク1' と表示される
  const tasks = query<ExistingTask>(
    "SELECT overrides_json FROM tasks WHERE preset_id = 18 AND (queue_name = 'default' OR queue_name = 'タスク1')",
    []
  );
  ...
  for (const task of tasks || []) {
    try {
      ...
      // プロフィール変更タスクは既存のコンテナを使用するため、container_idを確認
      // ただし、overridesにcontainer_idが含まれている可能性もある
      if (overrides.container_id) {
        containerIds.add(String(overrides.container_id));
      }
    } catch (e) {
      // JSON解析エラーは無視
    }
  ...
  // tasksテーブルのcontainer_idも確認（タスク1のみ）
  const tasksWithContainerId = query<{ container_id: string }>(
    "SELECT container_id FROM tasks WHERE preset_id = 18 AND container_id IS NOT NULL AND (queue_name = 'default' OR queue_name = 'タスク1')",
    []
  );
  for (const task of tasksWithContainerId) {
    containerIds.add(task.container_id);
  }
...
function getProfileData(): ProfileData[] {
  return query<ProfileData>(
    'SELECT container_id, name, bio, location, website, banner_image_path, avatar_image_path FROM profile_data ORDER BY container_id',
    []
  );
}
 ...
 * プロフィール変更タスクを作成
 * 
 * 注意: containerIdはUUID形式である必要があります
 * - 名前形式（例: infoborne113558）の場合は、Container Browser APIでエラーが発生します
 * - x_accountsテーブルのcontainer_idは既にUUID形式に更新されているため、そのまま使用します
 */
function createProfileUpdateTask(profileData: ProfileData): string | null {
  try {
    // container_idがUUID形式か確認（念のため）
    const isUuidFormat = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(profileData.container_id);
    if (!isUuidFormat) {
      console.warn(`  ⚠ container_idがUUID形式ではありません: ${profileData.container_id}`);
      console.warn(`     scripts/update-container-ids-to-uuid.ts を実行してUUID形式に変換してください`);
      return null;
    }

    // nameとbioが空文字列の場合は、undefinedに変換（空文字列を送るとUIで表示されない可能性があるため）
    const overrides: Record<string, string> = {
      location: profileData.location,
      website: profileData.website,
      banner_image_path: profileData.banner_image_path,
      avatar_image_path: profileData.avatar_image_path,
    };
    
    // nameとbioが空文字列でない場合のみ設定
    if (profileData.name && profileData.name.trim() !== '') {
      overrides.name = profileData.name;
    }
    if (profileData.bio && profileData.bio.trim() !== '') {
      overrides.bio = profileData.bio;
    }

    const runId = enqueueTask({
      presetId: 18, // プロフィール変更
      containerId: profileData.container_id, // 既存のコンテナを使用（UUID形式）
      overrides,
      waitMinutes: 10,
    }, 'default'); // タスク1（defaultキュー）に作成
    return runId;
  } catch (e: any) {
...
function main() {
  ...
  console.log('🔧 プロフィール変更タスクの作成を開始...\n');

  // 1. 今回登録した100件のアカウントIDを取得
  const targetAccountIds = getTargetAccountIds();
  console.log(`今回登録したアカウント数: ${targetAccountIds.size}件\n`);

  // 2. 既にタスクが作成されているアカウントを取得
  const existingTaskContainerIds = getExistingTaskContainerIds();
  console.log(`既存のプロフィール変更タスク数: ${existingTaskContainerIds.size}件\n`);

  // 3. プロフィール情報を取得
  const allProfileData = getProfileData();
  console.log(`プロフィール情報数: ${allProfileData.length}件\n`);

  // 4. 対象アカウントのプロフィール情報をフィルタ
  const targetProfileData = allProfileData.filter(
    (profile) =>
      targetAccountIds.has(profile.container_id) &&
      !existingTaskContainerIds.has(profile.container_id)
  );

  console.log(`対象アカウント数: ${targetProfileData.length}件\n`);

  if (targetProfileData.length === 0) {
    console.log('❌ 作成するタスクがありません');
    return;
  }
  ...
  console.log('📝 プロフィール変更タスクを作成中...\n');
  for (let i = 0; i < targetProfileData.length; i++) {
    const profileData = targetProfileData[i];
    const runId = createProfileUpdateTask(profileData);
    if (runId) {
      successCount++;
      if ((i + 1) % 10 === 0) {
        console.log(`  [${i + 1}/${targetProfileData.length}] 処理中...`);
      }
    } else {
      errorCount++;
    }
  ...
  console.log(`対象アカウント数: ${targetProfileData.length}件`);
  console.log(`✓ 作成成功: ${successCount}件`);
  console.log(`✗ エラー: ${errorCount}件`);
  console.log('='.repeat(60));

  // 6. 作成されたタスク数を確認
  const createdTasks = query<{ count: number }>(
    'SELECT COUNT(*) as count FROM tasks WHERE preset_id = 18'
  );
  console.log(`\nプロフィール変更タスクの総数: ${createdTasks[0]?.count || 0}件`);
