...
async function main() {
  ...
  console.log(`グループ「${groupName}」にメールアドレスを割り当てます...\n`);
  ...
  // 各コンテナに対してメールアドレスを割り当て
  const results: Array<{containerId: string, containerName: string, email: string, xAccountId: number}> = [];
  ...
  for (let i = 0; i < availableContainers.length; i++) {
    ...
    run(
      'UPDATE x_accounts SET email = ?, email_password = ?, updated_at = ? WHERE id = ?',
      [email, emailAccount.email_password, Date.now(), xAccount.id]
    );
  ...
  // 結果をまとめて表示
  console.log('--- 処理完了 ---');
  console.log(`割り当てたメールアドレス数: ${results.length}件\n`);
  
  results.forEach((r, i) => {
    console.log(`${i + 1}. ${r.containerName} (${r.containerId})`);
    console.log(`   メール: ${r.email}`);
    console.log(`   XアカウントID: ${r.xAccountId}\n`);
  });
