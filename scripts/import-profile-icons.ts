#!/usr/bin/env tsx
/**
 * Google Drive URLリストからプロフィール画像をインポート
 * 
 * 使用方法:
 *   tsx scripts/import-profile-icons.ts
 * 
...
async function main(): Promise<void> {
  ...
  if (!inputText || inputText.trim().length === 0) {
    console.error('使い方:');
    console.error('  tsx scripts/import-profile-icons.ts [URLリストファイル]');
    console.error('');
    console.error('例:');
    console.error('  tsx scripts/import-profile-icons.ts urls.txt');
    console.error('  echo "https://drive.google.com/file/d/xxx/view" | tsx scripts/import-profile-icons.ts');
    process.exit(1);
  }
  ...
  for (const icon of icons) {
    try {
      // 既に存在する場合はスキップ
      const existing = query('SELECT id FROM profile_icons WHERE file_id = ?', [icon.fileId]);
      ...
      // 新規挿入
      run(
        'INSERT INTO profile_icons (file_id, url, used, created_at) VALUES (?, ?, 0, ?)',
