/**
 * Xアカウントデータを一括登録するスクリプト
 * 
 * データ形式: XID:Xパスワード:メールアドレス:メールパスワード:2FAコード:Authトークン:ct0
 * 
 * 処理内容:
 * 1. x_accountsテーブルにアカウント情報を登録
 * 2. proxiesテーブルからランダムにプロキシを取得して割り当て（複数回使用可）
 * 3. email_accountsテーブルから未使用のメールアドレスを取得して割り当て
 *    ※CSVに含まれるメールアドレスは無視し、テーブルから取得したメールを使用
 * 
 * 使用方法:
 *   npx tsx scripts/register-accounts-and-create-login-tasks.ts
 */
...
function getRandomProxy(): string | null {
  const proxies = query<{ id: number; proxy_info: string }>(
    'SELECT id, proxy_info FROM proxies ORDER BY RANDOM() LIMIT 1',
    []
  );
 ...
 * 未使用のメールアカウントを取得
 */
function getUnusedEmailAccount(): { id: number; email: string; password: string } | null {
  const emailAccount = query<{ id: number; email_password: string }>(
 ...
 * メールアカウントを使用済みにマーク
 */
function markEmailAccountUsed(emailAccountId: number): void {
  run(
    'UPDATE email_accounts SET used_at = ? WHERE id = ?',
    [Date.now(), emailAccountId]
  );
}

/**
 * Xアカウントを登録し、プロキシとメールアドレスを割り当て
 */
function insertXAccount(
  data: XAccountData,
  proxyInfo: string | null,
  emailAccount: { id: number; email: string; password: string } | null
  ...
  // プロキシIDを取得（proxy_infoから）
  let proxyId: number | null = null;
  if (proxyInfo) {
    const proxy = query<{ id: number }>(
  ...
  try {
    run(
      ...
      [
        ...
        emailAccount ? `${emailAccount.email}:${emailAccount.password}` : null,
        proxyId,
        now,
        now,
      ]
    ...
    // メールアカウントを使用済みにマーク
    if (emailAccount) {
      markEmailAccountUsed(emailAccount.id);
    }

    return {
      success: true,
      message: `追加成功: ${data.xId} (プロキシ: ${proxyInfo || 'なし'}, メール: ${emailAccount?.email || 'なし'})`,
    };
  } catch (e: any) {
    return {
      success: false,
...
function main() {
  ...
  for (let i = 0; i < dataLines.length; i++) {
    ...
    if (!proxyInfo) {
      console.warn(`  ⚠ プロキシが見つかりません（プロキシなしで続行）`);
    }

    // メールアカウントを取得
    const emailAccount = getUnusedEmailAccount();
    if (!emailAccount) {
      console.warn(`  ⚠ 未使用のメールアカウントが見つかりません（メールなしで続行）`);
    }

    // Xアカウントを登録
    const result = insertXAccount(data, proxyInfo, emailAccount);
  ...
  console.log(`✓ アカウント登録成功: ${success}件`);
