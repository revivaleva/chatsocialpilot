...
async function main() {
  ...
  console.log(`\n更新対象ステップ:`);
  console.log(`  ステップ${profileStepIndex + 1}: プロフィールページにアクセス → container_nameをDBから取得`);
  console.log(`  ステップ${followerStepIndex + 1}: フォロワー数を確認 → フォロワー数とフォロー数を取得してDBに保存`);
  
  // ステップ1: container_nameをDBから取得してプロフィールページにアクセス
  const getContainerNameStep = {
    type: 'eval',
    description: 'コンテナ名を取得',
    code: `
      // containerIdからコンテナ名を取得する処理はサーバー側で行われるため、
      // ここではコンテナ名を取得するための準備を行う
      // 実際のコンテナ名は、タスク実行時にcontainerIdから取得される
      // このステップは、後続のnavigateステップで使用するためのコンテナ名を準備する
      return { ok: true, containerName: '{{container_name}}' };
    `,
    postWaitSeconds: 0
  };
  
  // ステップ2: プロフィールページにアクセス（コンテナ名を使用）
  ...
  try {
    ...
    console.log(`      注意: container_nameはタスク実行時にcontainerIdから自動取得されます`);
    ...
    console.log(`\n注意: フォロワー数とフォロー数の保存処理は、taskQueue.tsに追加する必要があります`);
  } catch (e: any) {
