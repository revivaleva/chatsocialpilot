...
try {
  ...
  // 古いタスクを削除
  console.log('\n古いタスク（queue_name="task1"）を削除中...');
  const deleteResult = db.prepare('DELETE FROM tasks WHERE runId LIKE ? AND queue_name = ?').run('follower-%', 'task1');
  console.log(`削除件数: ${deleteResult.changes}件`);
  
  const now = Date.now();
  const queueName = 'default'; // 正しいキュー名
  let createdCount = 0;
