 ...
 * 
 * 使用方法:
 *   tsx scripts/get-random-header-icon.ts
 *   tsx scripts/get-random-header-icon.ts --mark-used  # 取得と同時に使用済みマーク
 */
 ...
 * 未使用の画像をランダムに取得
 */
function getRandomUnusedIcon(markAsUsed: boolean = false): HeaderIcon | null {
  // 未使用の画像を取得
  const unusedIcons = query<HeaderIcon>(
    'SELECT id, file_id, url, used, used_at FROM header_icons WHERE used = 0 ORDER BY RANDOM() LIMIT 1'
  );
