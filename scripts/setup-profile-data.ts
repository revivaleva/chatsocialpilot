/**
 * プロフィール変更のタスク用にDBにプロフィール情報をセットするスクリプト
 * 
 * 処理内容:
 * 1. プロフィール情報を保存するテーブルを作成（存在しない場合）
 * 2. 今回登録した100件のアカウントに対してプロフィール情報を設定
 * 
 * 使用方法:
 *   npx tsx scripts/setup-profile-data.ts
 */
...
function createProfileDataTable() {
  ...
  if (tableExists.length === 0) {
    ...
    `);
    run(`
      CREATE INDEX IF NOT EXISTS idx_profile_data_container_id ON profile_data(container_id)
    `);
    console.log('✓ profile_dataテーブルを作成しました');
  } else {
    console.log('✓ profile_dataテーブルは既に存在します');
  }
}
 ...
 * 未使用のヘッダ画像をランダムに取得
 */
function getRandomHeaderIcon(): string | null {
  const icons = query<{ url: string }>(
    'SELECT url FROM header_icons WHERE used = 0 ORDER BY RANDOM() LIMIT 1'
  );
 ...
 */
function getRandomProfileIcon(): string | null {
  const icons = query<{ url: string }>(
    'SELECT url FROM profile_icons WHERE used = 0 ORDER BY RANDOM() LIMIT 1'
  );
  return icons.length > 0 ? icons[0].url : null;
}
 ...
 * 画像を使用済みにマーク
 */
function markIconAsUsed(iconUrl: string, type: 'header' | 'profile'): void {
  const now = Date.now();
  if (type === 'header') {
    run('UPDATE header_icons SET used = 1, used_at = ? WHERE url = ?', [now, iconUrl]);
  } else {
    run('UPDATE profile_icons SET used = 1, used_at = ? WHERE url = ?', [now, iconUrl]);
  }
}

/**
 * プロフィール情報を設定（画像はテーブルから取得）
 */
function setProfileData(containerId: string): void {
  const now = Date.now();
  
  // 画像をテーブルから取得
  const bannerImageUrl = getRandomHeaderIcon();
  const avatarImageUrl = getRandomProfileIcon();
  
  // 画像を使用済みにマーク
  if (bannerImageUrl) {
    markIconAsUsed(bannerImageUrl, 'header');
  }
  if (avatarImageUrl) {
    markIconAsUsed(avatarImageUrl, 'profile');
  }
  
  // プロフィール情報（locationとwebsiteは空文字列）
  const profileData = {
    name: '', // 空文字列 = 変更しない
    bio: '', // 空文字列 = 変更しない
    location: '', // 空文字列 = 空にする
    website: '', // 空文字列 = 空にする
    banner_image_path: bannerImageUrl || '', // ヘッダ画像URL（未使用の画像から取得）
    avatar_image_path: avatarImageUrl || '', // プロフィール画像URL（未使用の画像から取得）
  };

  // 既存のレコードを確認
  const existing = query<{ id: number }>(
    'SELECT id FROM profile_data WHERE container_id = ?',
    [containerId]
  );
  ...
  if (existing.length > 0) {
    // 更新
    run(
      'UPDATE profile_data SET name = ?, bio = ?, location = ?, website = ?, banner_image_path = ?, avatar_image_path = ?, updated_at = ? WHERE container_id = ?',
      [
        profileData.name,
        profileData.bio,
        profileData.location,
        profileData.website,
        profileData.banner_image_path,
        profileData.avatar_image_path,
        now,
        containerId,
      ]
    );
    ...
    // 新規作成
    run(
      'INSERT INTO profile_data (container_id, name, bio, location, website, banner_image_path, avatar_image_path, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        containerId,
        profileData.name,
        profileData.bio,
...
function main() {
  ...
  console.log('🔧 プロフィール情報テーブルのセットアップを開始...\n');

  // 1. テーブルを作成
  createProfileDataTable();

  // 2. 今回登録した100件のアカウントIDを取得
  const targetAccountIds = getTargetAccountIds();
  console.log(`\n今回登録したアカウント数: ${targetAccountIds.size}件\n`);

  // 3. x_accountsテーブルから対象アカウントを取得
  const allAccounts = query<XAccount>(
    'SELECT id, container_id FROM x_accounts ORDER BY id'
  );

  const targetAccounts = allAccounts.filter(account =>
    targetAccountIds.has(account.container_id)
  );

  console.log(`対象アカウント数: ${targetAccounts.length}件\n`);

  if (targetAccounts.length === 0) {
    console.log('❌ 対象アカウントが見つかりませんでした');
    return;
  }

  // 4. 各アカウントにプロフィール情報を設定
  let successCount = 0;
  let errorCount = 0;

  console.log('📝 プロフィール情報を設定中...\n');
  for (let i = 0; i < targetAccounts.length; i++) {
    const account = targetAccounts[i];
    try {
      setProfileData(account.container_id);
      successCount++;
      if ((i + 1) % 10 === 0) {
        console.log(`  [${i + 1}/${targetAccounts.length}] 処理中...`);
      }
    } catch (e: any) {
      errorCount++;
      console.error(`  ✗ エラー: ${account.container_id} - ${e?.message || String(e)}`);
    }
  }
  ...
  // 5. 設定されたデータの確認
  const profileDataCount = query<{ count: number }>(
    'SELECT COUNT(*) as count FROM profile_data WHERE container_id IN (SELECT container_id FROM x_accounts WHERE container_id IN (' +
      Array(targetAccountIds.size).fill('?').join(',') +
      '))',
    Array.from(targetAccountIds)
  );

  console.log(`\n設定済みプロフィール情報数: ${profileDataCount[0]?.count || 0}件`);

  console.log('\n💡 次のステップ:');
  console.log('  1. profile_dataテーブルのデータを確認・編集');
  console.log('  2. プロフィール変更タスクを作成');
}
