#!/usr/bin/env tsx
/**
 * グループのコンテナにプロフィール情報、プロフィール画像、ヘッダ画像を設定するタスクを登録（1件）
 * 
 * 使用方法:
 *   tsx scripts/create-profile-task-for-group.ts <グループ名>
 * 
 * 例:
 *   tsx scripts/create-profile-task-for-group.ts "X兵隊12/8作成、プロフィール未変更"
 */
...
async function main(): Promise<void> {
  ...
  if (!groupName) {
    console.error('使い方: tsx scripts/create-profile-task-for-group.ts <グループ名>');
    console.error('');
    console.error('例:');
    console.error('  tsx scripts/create-profile-task-for-group.ts "X兵隊12/8作成、プロフィール未変更"');
    process.exit(1);
  }
  ...
  if (unregisteredContainers.length === 0) {
    console.log('すべてのコンテナにタスクが登録済みです。');
    process.exit(0);
  }
  ...
  for (let i = 0; i < unregisteredContainers.length; i++) {
    ...
    try {
      ...
      if (!profileTemplate) {
        console.error(`\nエラー: 未使用のプロフィールテンプレートが見つかりませんでした（${i + 1}/${unregisteredContainers.length}件目）`);
        errorCount++;
        break;
      }
      ...
      if (!profileIcon) {
        console.error(`\nエラー: 未使用のプロフィール画像が見つかりませんでした（${i + 1}/${unregisteredContainers.length}件目）`);
        errorCount++;
        break;
      }
      ...
      if (!headerIcon) {
        console.log(`\n未使用のヘッダ画像がなくなりました。フラグをリセットして再利用します...`);
        ...
        if (!headerIcon) {
          console.error(`\nエラー: ヘッダ画像が見つかりませんでした（${i + 1}/${unregisteredContainers.length}件目）`);
          errorCount++;
          break;
        }
      }

      console.log(`タスク ${i + 1}/${unregisteredContainers.length}:`);
