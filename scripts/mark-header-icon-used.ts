 ...
 * 
 * 使用方法:
 *   tsx scripts/mark-header-icon-used.ts <file_id or url>
 * 
 * 例:
 *   tsx scripts/mark-header-icon-used.ts 1dbcJxxIHx86RyLMwZNN0DGi3sy-BGHG6
 *   tsx scripts/mark-header-icon-used.ts "https://drive.google.com/file/d/1dbcJxxIHx86RyLMwZNN0DGi3sy-BGHG6/view"
 */
...
async function main(): Promise<void> {
  ...
  if (!input) {
    console.error('使い方: tsx scripts/mark-header-icon-used.ts <file_id or url>');
    console.error('');
    console.error('例:');
    console.error('  tsx scripts/mark-header-icon-used.ts 1dbcJxxIHx86RyLMwZNN0DGi3sy-BGHG6');
    console.error('  tsx scripts/mark-header-icon-used.ts "https://drive.google.com/file/d/1dbcJxxIHx86RyLMwZNN0DGi3sy-BGHG6/view"');
    process.exit(1);
  }
  ...
  const icons = query<{ id: number; file_id: string; url: string; used: number }>(
    'SELECT id, file_id, url, used FROM header_icons WHERE file_id = ?',
    [fileId]
  );
