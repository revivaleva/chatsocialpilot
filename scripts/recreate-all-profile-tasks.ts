...
function main() {
  ...
  if (!groupName) {
    console.error('使い方: tsx scripts/recreate-all-profile-tasks.ts [グループ名]');
    console.error('');
    console.error('例:');
    console.error('  tsx scripts/recreate-all-profile-tasks.ts "X兵隊12/8作成、プロフィール未変更"');
    process.exit(1);
  }
